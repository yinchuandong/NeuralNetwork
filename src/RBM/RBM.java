...
public class RBM {
	...
	public RBM(int N, int n_visible, int n_hidden, double[][] W,
	...
	public void contrastive_divergence(int[] input, double lr, int k) {
		...
		for (int step = 0; step < k; step++) {
			if (step == 0) {
				...
				gibbs_hvh(nh_samples, nv_means, nv_samples, nh_means,
						nh_samples);
			}
		}
...
//						/ N;
	...
	/**
	 * 网络向上传播，给定visible layer，计算hidden layer
	 * 
	 * @param v0_sample
	 * @param mean
	 * @param sample
	...
	/**
	 * 吉布斯采样<br/>
	 * 1.可视单元给隐藏单元，新的可视单元由隐藏单元重建<br/>
	 * 2.新的可视单元再次给隐藏单元，得到新的隐藏单元
	 * 
	 * @param h0_sample
	 * @param nv_means
	 * @param nv_samples
	 ...
	 * @param nh_samples
	 */
	public void gibbs_hvh(int[] h0_sample, double[] nv_means, int[] nv_samples,
